...
RSpec.feature 'Admin visits dashboard' do
  ...
  scenario 'admin sees listing for all orders' do
    ...
    expect(page).to have_link(order1.id.to_s, href: "/orders/#{order1.id}")
    expect(page).to have_link(order2.id.to_s, href: "/orders/#{order2.id}")
    expect(page).to have_link(order3.id.to_s, href: "/orders/#{order3.id}")
    expect(page).to have_link(order4.id.to_s, href: "/orders/#{order4.id}")
