...
RSpec.describe 'Admin can visit profile page' do
  ...
  scenario 'and they can see their credentials' do
    ...
    fill_in 'session[email]',    with: admin.email.to_s
    fill_in 'session[password]', with: admin.password.to_s
  ...
  it 'and they can edit their credentials' do
    ...
    expect(page).to have_content("Welcome, #{admin.first_name}")
    expect(find_field('Email').value).to eq(admin.email.to_s)
    expect(find_field('First name').value).to eq(admin.first_name.to_s)
    expect(find_field('Last name').value).to eq(admin.last_name.to_s)
    expect(find_field('Address').value).to eq(admin.address.to_s)
